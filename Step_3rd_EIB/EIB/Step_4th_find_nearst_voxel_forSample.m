...
atlas_246= load_nii('BN_Atlas_246_2mm.nii');%load_nii is a function located in '\functions\NIfTI_20140122'
atlas_246.hdr.hist
atlas_246_zero_point = atlas_246.hdr.hist.originator(:,1:3)
atlas_246_mat = atlas_246.img;

% load('left_ROI_list_AAL625.mat')
% load('information_BN246MASK.mat')
left_ROI_list=[1:2:246]'%左半脑 BN_246 奇数 1 3 5 7....
[left_ROI_voxel_MNIcorr_label,left_ROI_voxel_index_label] = voxel_MNI(atlas_246_mat,atlas_246_zero_point,left_ROI_list);
...
for i = 1:size(SampleAnnot_only_left,1)
    ...
    for j = 1:sample_counts
        ...
        if nearst_dist < 2 %the smallest distance under 2 mm theshold
            if size(nearst_voxel_mni_label,1)==1
                ...
                if size(specfic_roi_label,1) == 1
                    ...
                    for k = 1:size(specfic_roi_label,1)
                        ...
                        all_vol_of_roi_k = size(find(atlas_246_mat == roi_label),1);
                        %check
            ...
            %did not find the voxel with smallest distance under 2 mm theshold
